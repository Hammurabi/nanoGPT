...
class Neuron(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc = nn.Linear(
            config.n_embd, config.p_expert, bias=config.bias)
        self.gelu = nn.GELU()
        self.c_proj = nn.Linear(
            config.p_expert, config.n_embd, bias=config.bias)
        self.dropout = nn.Dropout(config.dropout)
...
class SelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Using PyTorch's MultiheadAttention module
        ...
        self.ln = nn.LayerNorm(config.n_embd, elementwise_affine=config.bias)

    def forward(self, x):
        """
        x: Tensor of shape (batch, sequence_length, n_embd)
        """
        # Pre-normalize inputs
        x_norm = self.ln(x)
        # PyTorch's MultiheadAttention expects input shape (seq_len, batch, embed_dim)
        # so we transpose the input from (batch, seq_len, embed_dim) to (seq_len, batch, embed_dim)
        x_transposed = x_norm.transpose(0, 1)
        # Self-attention: query, key, and value are all x_transposed
        attn_output, _ = self.attn(x_transposed, x_transposed, x_transposed)
        # Transpose back to original shape: (batch, seq_len, embed_dim)
...
class GPTConfig:
    ...
    p_expert: int = 32 # an expert is a perceptron
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.vision = nn.ModuleDict(dict(
            wte=nn.Embedding(config.vocab_size, config.cortical_neurons),
            wpe=nn.Embedding(config.block_size, config.cortical_neurons),
            drop=nn.Dropout(config.dropout),
            lc=nn.Linear(config.cortical_neurons, config.n_embd)
        ))

        self.feeling = nn.Linear(config.n_feelings, config.n_embd)

        self.sensing = nn.ModuleDict(dict(
            wte=nn.Embedding(config.sense_vocab, config.n_embd),
            wpe=nn.Embedding(config.body_parts, config.n_embd),
            drop=nn.Dropout(config.dropout),
        ))
    ...
    def forward(self, idx, fov, feelings, senses, sense_ids, targets=None):
        device = idx.device
        b, t = idx.size()
        ...
        # feelings
        _feelings = self.feeling(feelings)
        # x = self.transformer.drop(tok_emb + pos_emb + _feelings)

        fov_emb = self.vision.wte(fov)
        pix = torch.arange(0, fov.size()[1], dtype=torch.long, device=device)  # shape (t)
        pix_emb = self.vision.wpe(pix)

        senses_emb = self.sensing.wte(senses)
        senseid_emb  = self.sensing.wpe(sense_ids)

        combined_embeds = torch.cat((tok_emb, pos_emb, fov_emb, pix_emb, senses_emb, senseid_emb), dim=1)
        # x = self.transformer.drop(_feelings + combined_embeds)
        expanded_feelings = _feelings.unsqueeze(1)  # shape becomes [b, 1, n_embd]
        expanded_feelings = expanded_feelings.expand(-1, combined_embeds.size(1), -1)  # shape [b, sequence_length, n_embd]
        x = self.transformer.drop(expanded_feelings + combined_embeds)
